...
def circular_hard_pupil(xx, yy, radius_m):
    """
    thin_lens_transmittance() is, by default, phase-only. In other words, it doesn't have hard aperture.
    This function is used to create hard aperture and multiply it with the output of thin_lens_transmittance()
    to create a physical lens transmittance.
    """
    rr = np.sqrt(xx**2 + yy**2)
    return (rr <= radius_m).astype(np.float64)


def sag_sphere(r: np.ndarray, R: float) -> np.ndarray:
    """
    sag = R - sign(R)*sqrt(R^2 - r^2)
    """
    Rabs2 = (abs(R) ** 2)
    inside = Rabs2 - r * r
    # For numerical stability: clamp small negative due to float error.

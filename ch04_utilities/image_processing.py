...
def pad_with_zeros(Uin, pad_ratio=None, pad_size=None):
    """
    Zero-pad a square input array (2D or 3D) symmetrically.

    [inputs]
    Uin : np.ndarray
        Input array of shape (M0, M0) or (M0, M0, C).
        Can be real or complex.

    pad_ratio : float, optional
        Scaling factor for padding. If provided, overrides pad_size.
        Example: pad_ratio=2.0 → output size = 2*M0

    pad_size : int, optional
        Number of pixels to add to each dimension (total size increase).
        Example: pad_size=100 → output size = M0 + 100
    ...
    # Input validation
    # ------------------------------------------------------------
    if pad_ratio is None and pad_size is None:
        raise ValueError("pad_ratio and pad_size cannot both be None.")

    if Uin.ndim not in [2, 3]:
        raise ValueError("Uin must be 2D (monochrome) or 3D (e.g., RGB).")

    M0 = Uin.shape[0]

    if Uin.shape[1] != M0:
        raise ValueError("Uin must be square (M0 x M0).")

    # ------------------------------------------------------------
    # Determine output size
    # ------------------------------------------------------------
    if pad_ratio is not None:
        M = int(np.round(M0 * pad_ratio))
    else:
        M = M0 + int(pad_size)

    if M < M0:
        raise ValueError("Padding must not reduce size.")
    ...
    else:  # 3D (e.g., RGB or multi-channel)
        pad_width = ((pad_before, pad_after),
                     (pad_before, pad_after),
                     (0, 0))  # no padding along channel dimension

    U_padded = np.pad(Uin, pad_width, mode='constant', constant_values=0)

    return U_padded, M, M0

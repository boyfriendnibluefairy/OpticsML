...
def _save_np_core(U_np,
    ...
    [inputs]
        ...
        title       : figure title (ignored if strip_labels=True)
        cmap        : matplotlib colormap
        strip_labels: if True, remove titles, axes, ticks, borders, and colorbars
    """
    ...
    # Create figure without specifying figsize → avoids extra border padding
    fig = plt.figure()
    ...
    # Save image with zero borders
    fig.savefig(full_path, dpi=300, pad_inches=0, bbox_inches="tight")
    plt.close(fig)

...
def save_extracted_sub_images(
    ...
    """
    [inputs]
    outputs of extract_field_points() and:
      1) resize each to (resize x resize)
      2) save them as:
         - original_with_box_marks.png
         - field0.png
         - field7.png
         - field10.png
    Images are saved in the same directory as the *calling* python script.
    """
    if not isinstance(resize, int) or resize <= 0:
        raise ValueError("resize must be a positive integer (pixels).")
    ...
    def _to_uint8_for_save(im: np.ndarray) -> np.ndarray:
        if im.dtype == np.uint8:
            return im

        imf = im.astype(np.float32)
        imf = np.nan_to_num(imf, nan=0.0, posinf=0.0, neginf=0.0)

        mn, mx = float(imf.min()), float(imf.max())
        if mx > mn:
            imf = (imf - mn) / (mx - mn)
        else:
            imf = np.zeros_like(imf)
    ...
    def _resize(im: np.ndarray) -> np.ndarray:
        if im.ndim == 2:
            return cv2.resize(im, (resize, resize), interpolation=cv2.INTER_AREA)
        if im.ndim == 3:
            return cv2.resize(im, (resize, resize), interpolation=cv2.INTER_AREA)
        raise ValueError("Unsupported image ndim; expected 2D or 3D array.")

    def _rgb_to_bgr_for_imwrite(im_u8: np.ndarray) -> np.ndarray:
        """
        cv2.imwrite expects BGR for 3-channel images.
        Convert RGB->BGR if color; pass-through if grayscale.
        """
        if im_u8.ndim == 2:
            return im_u8
        if im_u8.ndim == 3:
    ...
    # Resize
    box_marks_r = _resize(box_marks)
    field0_r = _resize(field0)
    field7_r = _resize(field7)
    field10_r = _resize(field10)

    # uint8
    box_marks_u8 = _to_uint8_for_save(box_marks_r)
    field0_u8 = _to_uint8_for_save(field0_r)
    field7_u8 = _to_uint8_for_save(field7_r)
    field10_u8 = _to_uint8_for_save(field10_r)

    # Save with correct channel order
    cv2.imwrite(str(out_dir / "original_with_box_marks.png"), _rgb_to_bgr_for_imwrite(box_marks_u8))
    cv2.imwrite(str(out_dir / "field0.png"), _rgb_to_bgr_for_imwrite(field0_u8))
    cv2.imwrite(str(out_dir / "field7.png"), _rgb_to_bgr_for_imwrite(field7_u8))
    cv2.imwrite(str(out_dir / "field10.png"), _rgb_to_bgr_for_imwrite(field10_u8))

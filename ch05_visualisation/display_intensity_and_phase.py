"""
    file : display_intensity_and_phase.py
"""
...
def _to_rgb_np(U, expect_complex=None, func_name="display_RGB"):
    """
    Convert input to an RGB NumPy array with shape (H, W, 3).
    ...
    [inputs]
    expect_complex : bool or None
        - True  -> require complex-valued input
        - False -> require real-valued input
        - None  -> allow both
    """
    # torch -> numpy
    if isinstance(U, torch.Tensor):
        ...
        raise TypeError(f"U must be a NumPy ndarray or torch.Tensor for {func_name}().")

    # Accept (H,W,3) or (3,H,W)
    if U_np.ndim != 3:
        raise ValueError(f"{func_name}() expects a 3D RGB array/tensor, got shape {U_np.shape}.")

    if U_np.shape[-1] == 3:
        pass  # already (H, W, 3)
    elif U_np.shape[0] == 3:
        U_np = np.transpose(U_np, (1, 2, 0))  # (3, H, W) -> (H, W, 3)
    else:
        raise ValueError(
            f"{func_name}() expects RGB data with shape (H, W, 3) or (3, H, W), got {U_np.shape}."
        )

    is_complex = np.iscomplexobj(U_np)

    if expect_complex is True and not is_complex:
        raise TypeError(f"{func_name}() expects a complex-valued RGB array/tensor.")
    if expect_complex is False and is_complex:
        raise TypeError(f"{func_name}() expects a real-valued RGB array/tensor.")

    return U_np


def _display_rgb_np_core(U_rgb_np,
                         intensity=True,
                         phase=False,
                         title="RGB Intensity and Phase",
                         filename=None,
                         cmap_intensity="gray",
                         cmap_phase="twilight"):
    """
    Internal RGB display core.

    U_rgb_np must be shape (H, W, 3), real or complex.
    """
    if not intensity and not phase:
        raise ValueError("At least one of `intensity` or `phase` must be True.")

    if not isinstance(U_rgb_np, np.ndarray):
        raise TypeError("U_rgb_np must be a NumPy ndarray.")

    if U_rgb_np.ndim != 3 or U_rgb_np.shape[2] != 3:
        raise ValueError(f"U_rgb_np must have shape (H, W, 3), got {U_rgb_np.shape}.")
    ...
    if intensity:
        I_list = [np.abs(U_rgb_np[:, :, c]) ** 2 for c in range(3)]

    if phase:
        Phi_list = [np.angle(U_rgb_np[:, :, c]) for c in range(3)]
    ...
    if intensity and phase:
        ...
        for c in range(3):
            # intensity row
            imI = axes[0, c].imshow(I_list[c], cmap=cmap_intensity)
            axes[0, c].set_title(f"{channel_names[c]} Intensity")
            axes[0, c].set_xlabel("x (pixels)")
            axes[0, c].set_ylabel("y (pixels)")
            fig.colorbar(imI, ax=axes[0, c], fraction=0.046, pad=0.04)

            # phase row
            imP = axes[1, c].imshow(Phi_list[c], cmap=cmap_phase, vmin=-np.pi, vmax=np.pi)
            axes[1, c].set_title(f"{channel_names[c]} Phase")
            axes[1, c].set_xlabel("x (pixels)")
            axes[1, c].set_ylabel("y (pixels)")
            fig.colorbar(imP, ax=axes[1, c], fraction=0.046, pad=0.04)

    elif intensity:
        fig, axes = plt.subplots(1, 3, figsize=(16, 5))
        for c in range(3):
            im = axes[c].imshow(I_list[c], cmap=cmap_intensity)
            axes[c].set_title(f"{channel_names[c]} Intensity")
            axes[c].set_xlabel("x (pixels)")
            axes[c].set_ylabel("y (pixels)")
            fig.colorbar(im, ax=axes[c], fraction=0.046, pad=0.04)

    else:  # phase only
        fig, axes = plt.subplots(1, 3, figsize=(16, 5))
        for c in range(3):
            im = axes[c].imshow(Phi_list[c], cmap=cmap_phase, vmin=-np.pi, vmax=np.pi)
            axes[c].set_title(f"{channel_names[c]} Phase")
            axes[c].set_xlabel("x (pixels)")
            axes[c].set_ylabel("y (pixels)")
            fig.colorbar(im, ax=axes[c], fraction=0.046, pad=0.04)
...
def display_rgb01(rgb01: np.ndarray, title: str = "RGB") -> None:
    import matplotlib.pyplot as plt
...
def display_RGB_real_tensor(U,
                            ...
                            cmap="gray"):
    """
    Display a real-valued RGB PyTorch tensor.
    ...
    """
    if torch is None:
        raise ImportError("PyTorch is not available, but display_RGB_real_tensor was called.")

    if not isinstance(U, torch.Tensor):
        raise TypeError("U must be a torch.Tensor for display_RGB_real_tensor().")

    if torch.is_complex(U):
        raise TypeError("display_RGB_real_tensor() expects a real-valued RGB tensor.")

    U_np = _to_rgb_np(U, expect_complex=False, func_name="display_RGB_real_tensor")
    U_np = U_np.astype(np.float64)

    _display_rgb_np_core(
        U_np,
        intensity=intensity,
        phase=phase,
        title=title,
...
def display_RGB_complex_tensor(U,
                               intensity=True,
                               phase=False,
                               title=None,
                               filename=None,
                               cmap="gray"):
    """
    Display a complex-valued RGB PyTorch tensor.
    ...
    """
    if torch is None:
        raise ImportError("PyTorch is not available, but display_RGB_complex_tensor was called.")

    if not isinstance(U, torch.Tensor):
        raise TypeError("U must be a torch.Tensor for display_RGB_complex_tensor().")

    if not torch.is_complex(U):
        raise TypeError("display_RGB_complex_tensor() expects a complex-valued RGB tensor.")

    U_np = _to_rgb_np(U, expect_complex=True, func_name="display_RGB_complex_tensor")
...
def display_RGB_real_np(U,
                        ...
                        cmap="gray"):
    """
    Display a real-valued RGB NumPy array.
    ...
    """
    if not isinstance(U, np.ndarray):
        raise TypeError("U must be a NumPy ndarray for display_RGB_real_np().")

    if np.iscomplexobj(U):
        raise TypeError("display_RGB_real_np() expects a real-valued RGB array.")

    U_np = _to_rgb_np(U, expect_complex=False, func_name="display_RGB_real_np")
    U_np = U_np.astype(np.float64)
...
def display_RGB_complex_np(U,
                           ...
                           cmap="gray"):
    """
    Display a complex-valued RGB NumPy array.
    ...
    """
    if not isinstance(U, np.ndarray):
        raise TypeError("U must be a NumPy ndarray for display_RGB_complex_np().")

    if not np.iscomplexobj(U):
        raise TypeError("display_RGB_complex_np() expects a complex-valued RGB array.")

    U_np = _to_rgb_np(U, expect_complex=True, func_name="display_RGB_complex_np")
